      ...
      {conversation.length === 0 && !isAISpeaking && !isUserSpeaking ? (
        ...
        <>
          {conversation.map((item, index) => (
            <div key={`${item.role}-${index}`} className={`p-3 rounded-lg max-w-[90%] w-fit ${item.role === 'ai' ? 'bg-blue-100 self-start' : 'bg-green-100 self-end'}`}>
              <div className="font-bold">{item.role === 'ai' ? 'Interviewer' : 'You'}</div>
              <div>{item.text}</div>
            </div>
          ))}
